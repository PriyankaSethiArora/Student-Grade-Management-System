...
def calculate_average(students):
    ...
    if name in students and students[name]:
        ...
        print(f"{name}'s average grade: {avg:.2f} ({letter_grade})")
    else:
...
def find_extremes(students):
    ...
    print(f"Highest scoring student: {highest} with {averages[highest]:.2f} ({get_letter_grade(averages[highest])})")
    print(f"Lowest scoring student: {lowest} with {averages[lowest]:.2f} ({get_letter_grade(averages[lowest])})")
...
def main():
    ...
    while True:
        ...
        print("4. Find Highest and Lowest Scoring Students")
        print("5. Exit")
